...
1. **Model surgery: inject the reservoir into a small pretrained transformer (H1).**
...
2. **Dynamics sweep on REAL attention streams + write up.** Drive the sweep (item 4) with
...
3. **Ambitious reach (per `todo.md` §B note — "just to see if we can").** Attempt the
