...
- **Fix 1 — gate-D (unresolved-thread) F1 reconciled from a real run (for arXiv v2).** Ran the authorized canonical config `python scripts/run.py silence` (K=300, T=4000, speak-window=5, ρ=0.9, input-scaling=0.5, seed 0). Authoritative numbers from `results/silence_gate.json`: reservoir P=0.926, R=1.000, **F1=0.962**; stateless P=0.205, R=0.965, **F1=0.338** (speak base rate 0.206). So the 0.96/0.34 version (summary line ~752 + Appendix E) was correct; the 0.48/0.03 version in the main H4(D) section was a stale paste and is overwritten. The stateless failure mode is **"always speaks"** (recall ≈ 1, precision ≈ base rate), not "misses the thread" — the difference is *precision*, not recall; flipped the narrative accordingly in FINDINGS.md and docs/index.html. Ran seeds 1–2 to confirm robustness (reservoir F1 0.96–0.99, stateless 0.34–0.45) and restored the seed-0 canonical figure. Historical 0.48/0.03 mention in this devlog left as-is (append-only trail).
- **Fix 2 — KV-append vs KV-prefix terminology reconciled (no run; source code settles it).** Source confirms three distinct paths: additive residual-stream write (wired, ignored at chance), **KV-prefix** (state → prefix pseudo-tokens via `inputs_embeds`, the path actually run → 100% recall; `kv_live.py`), and strict mid-layer **KV-append** (reservoir rows appended into a layer's K/V; implemented + unit-tested in isolation only, no HF `generate` hook → integration blocker; `kv_inject.py:85`). Removed the wrong "we use KV-append and KV-prefix interchangeably for the same injection" line (FINDINGS ~450) and the "Content-addressable (KV-append) → works" mislabel; relabeled the working path KV-prefix everywhere; rewrote Appendix A to name all three paths and attribute the 100% recall to KV-prefix (it previously listed only two and implied KV-append delivered it). Mirrored the docs/index.html "KV-append fix" → "KV-prefix path" mislabel. `paper/reviews/*` left untouched (historical reviewer correspondence). Paper PDF rebuilds from FINDINGS.md via `paper-pdf.yml` (pandoc → paper.tex.body); FINDINGS.md remains the single source.
