...
3. **Go live: create a PUBLIC GitHub repo and push.** Public is required for free GitHub Pages. `gh repo create --public --source=. --push`. Confirm CI (`.github/workflows/`) is wired and set **Settings → Pages → Source: GitHub Actions** so `docs/` (the report site) and the built PDF deploy. From here every commit pushes and Pages/CI build as you go.
