...
2. **Agent harness — the always-alive runtime (PRIORITY; before any fine-tuning).**
...
3. **Input-scaling tuning sweep (follow-up to the over-saturation finding).** The real
...
4. **Theory section (`todo.md` §C), correctly scoped.** Write the formal claims into a
...
5. **Train a readout for H3 (state is informative).** Build a small reservoir-requiring
...
6. **KV-append injection variant + H1 regression.** Implement the richer injection:
...
7. **Citation-checked novelty follow-up (`todo.md` §D).** Run a focused, verified review
