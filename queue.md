...
1. **Reservoir core (TDD).** `src/reservoir/echo_state.py`: fixed sparse `W_r` scaled to a
...
2. **Reservoir observability / dynamics metrics (TDD).** `src/reservoir/metrics.py`:
...
3. **Spectral-radius dynamics sweep on synthetic input.** `scripts/run.py` drives the
...
4. **Model surgery: inject the reservoir into a small pretrained transformer (H1).**
...
5. **Dynamics sweep on REAL attention streams + write up.** Drive the sweep (item 4) with
...
6. **Ambitious reach (per `todo.md` §B note — "just to see if we can").** Attempt the
